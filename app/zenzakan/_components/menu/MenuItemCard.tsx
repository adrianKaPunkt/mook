...
export default function MenuItemCard({ item, locale }: { item: MenuItem; locale: "de" | "en" }) {
  ...
  return (
    <div className="relative border border-t-primary px-2 py-6 lg:py-9 transition-all duration-300 hover:scale-102 group cursor-pointer">
      {/* New Badge */}
      {isNew && (
      ...
      <div className="flex justify-between items-start gap-4">
        <div className="flex-1 min-w-0">
          {/* Name */}
          <h3 className="flex gap-4 text-white/90 font-light uppercase font-heading text-xl lg:text-3xl leading-snug mb-1 group-hover:text-white transition-colors duration-300">
            {item.name}
          ...
          {item.allergens && item.allergens.length > 0 && (
            <div className="mt-5 flex flex-wrap gap-2">
              {item.allergens.map((allergenKey) => {
                ...
                const isOpen = activeAllergen === allergenKey;

                return (
                  <button
                    key={allergenKey}
                    type="button"
                    onClick={() => setActiveAllergen(isOpen ? null : allergenKey)}
                    className="inline-flex max-w-full items-center gap-1.5 py-1 text-base text-foreground/80 cursor-pointer"
                  >
                    {allergenData?.imageUrl && (
                      <Image
                        src={allergenData.imageUrl}
                        alt={name ?? allergenKey}
                        width={16}
                        height={16}
                        className="size-8 lg:size-10 shrink-0 invert"
                      />
                    )}
                    <span
                      className={`truncate uppercase transition-all duration-200 ${isOpen ? "block" : "hidden lg:block"}`}
                    >
                      {name ?? allergenKey}
                    </span>
                  </button>
